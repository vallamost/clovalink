...
check_secret_strength() {
    ...
    # Check for exact matches of known insecure defaults
    if [ "$var_value" == "dev-secret-change-in-production" ] || \
       [ "$var_value" == "change-me" ] || \
       [ "$var_value" == "your-secret-here" ] || \
       [ "$var_value" == "example-secret" ]; then
        echo -e "${RED}✗ $var_name uses a known default/example value${NC}"
        ERRORS=$((ERRORS + 1))
    fi
}
